 ...
 */
export function Tabs({ items, ariaLabel, className }) {
    return (_jsx("nav", { "aria-label": ariaLabel, className: cx("flex items-center gap-1", className), children: items.map((item) => {
            const cls = cx(TAB, item.active ? TAB_ON : TAB_OFF);
            const inner = (_jsxs(_Fragment, { children: [item.label, item.count != null && (_jsxs(_Fragment, { children: [" ", _jsxs("span", { className: "tnum text-muted2", children: ["\u00B7 ", item.count] })] }))] }));
            return item.href ? (_jsx(Link, { href: item.href, "aria-current": item.active ? "page" : undefined, className: cls, children: inner }, item.href)) : (_jsx("button", { type: "button", "aria-pressed": !!item.active, onClick: item.onClick, className: cls, children: inner }, item.label));
        }) }));
}
