    ...
    <aside className="hidden w-[230px] shrink-0 lg:block">
      <div className="sticky top-0 flex h-screen flex-col px-4 py-5">
        ...
        <div className="relative mt-9 flex flex-1 flex-col gap-5">
          {sections.map((s, i) => (
            <nav key={s.label ?? i} aria-label={s.label} className="flex flex-col gap-1">
              {s.label && <div className="eyebrow mb-1 px-3">{s.label}</div>}
              {s.items.map(({ label, href, icon, badge, exact }) => {
